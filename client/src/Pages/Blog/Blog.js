				...
				<>
					<TextContainer content={
						...
						<form action={createmethod + '/' +blogPosts[i].slug_title +"?_method=DELETE"} method="POST">
							<label><div style={{maxWidth:"50%",display:"inline-block"}}><a href={"../../blog/" + blogPosts[i].slug_title}>{(blogPosts[i].slug_title).replaceAll('-',' ')}</a></div><div style={{maxWidth:"50%",float:"right"}}><span style={{color:'darkgray'}}>{blogPosts[i].author}</span></div><br/>{props.auth ? " - " : "" }</label>
							{props.auth ?<input type="submit" value="delete"/> : <></>}
						</form>
						<br/>
						</>
